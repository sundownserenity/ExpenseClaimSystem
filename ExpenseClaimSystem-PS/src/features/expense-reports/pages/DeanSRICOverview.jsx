...
const DeanSRICOverview = () => {
  ...
  const fetchDashboardStats = async () => {
    setIsLoading(true);
    try {
      // Fetch pending reports (Project Fund reports at School Chair Approved status)
      const { data: pendingData } = await API.get('/expense-reports?pending=true');
      
      // Fetch processed reports (Project Fund reports that have been processed)
      const { data: processedData } = await API.get('/expense-reports?processed=true');
      
      // Calculate statistics
      const approved = processedData.filter(r => 
        r.deanSRICApproval?.approved === true
      );
      
      const rejected = processedData.filter(r => 
        r.status === 'Rejected' && r.deanSRICApproval?.approved === false
      );
      ...
      // Calculate total amount approved this month by this Dean SRIC
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      const approvedThisMonth = approved.filter(r => {
        if (!r.deanSRICApproval?.date) return false;
        const approvalDate = new Date(r.deanSRICApproval.date);
        return approvalDate.getMonth() === currentMonth && 
               approvalDate.getFullYear() === currentYear &&
               r.deanSRICApproval.approvedById === user._id;
      });
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div 
          ...
          onClick={() => navigate('/expense-reports/dean-sric/approvals')}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() => navigate('/expense-reports/dean-sric/approvals?tab=processed')}
              ...
              stats.recentReports.map((report) => (
                ...
                >
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(report.status)}`}>
                    {report.status === 'School Chair Approved' ? 'Pending' : report.status}
                  </span>
                </div>
              ))
            )}
      ...
      {/* Role Info */}
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg shadow-sm border border-purple-100 p-6">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Dean SRIC</h3>
            <p className="text-sm text-gray-600">Sponsored Research & Industrial Consultancy</p>
          </div>
        </div>
      </div>
    </div>
...
export default DeanSRICOverview;
