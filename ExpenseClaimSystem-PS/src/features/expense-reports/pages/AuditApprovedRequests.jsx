...
const AuditApprovedRequests = () => {
  ...
  const fetchApprovedReports = async () => {
    ...
    try {
      ...
      // Filter to show only reports approved by this Audit user
      const myApprovedReports = data.filter(report => {
        const auditApproval = report.auditApproval;
        return (report.status === 'Audit Approved' || report.status === 'Finance Approved') &&
               auditApproval && 
  ...
  const filterReports = () => {
    ...
    if (dateFilter !== 'all') {
      const now = new Date();
      filtered = filtered.filter(report => {
        const approvalDate = new Date(report.auditApproval.date);
        switch (dateFilter) {
          case 'today':
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Audit - Approved Requests</h1>
        <p className="text-gray-600 mt-2">
          Expense reports you have approved during audit review
        </p>
      </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredReports.map((report) => (
                  <tr key={report._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {formatDate(report.auditApproval.date)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
...
export default AuditApprovedRequests;
