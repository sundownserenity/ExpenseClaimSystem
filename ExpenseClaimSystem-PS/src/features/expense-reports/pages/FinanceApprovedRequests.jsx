...
const FinanceApprovedRequests = () => {
  ...
  const fetchApprovedReports = async () => {
    ...
    try {
      ...
      // Filter to show only reports approved by this Finance user
      const myApprovedReports = data.filter(report => {
        const financeApproval = report.financeApproval;
        return report.status === 'Finance Approved' &&
               financeApproval && 
  ...
  const filterReports = () => {
    ...
    if (dateFilter !== 'all') {
      const now = new Date();
      filtered = filtered.filter(report => {
        const approvalDate = new Date(report.financeApproval.date);
        switch (dateFilter) {
          case 'today':
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">
          Requests you have approved as Finance
        </p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="text-2xl font-bold text-gray-700">
            {approvedReports.length > 0 
              ? formatCurrency(approvedReports.reduce((sum, r) => sum + (r.totalAmount || 0), 0) / approvedReports.length)
              : formatCurrency(0)}
          </div>
        </div>
      </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredReports.map((report) => (
                  <tr key={report._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {formatDate(report.financeApproval.date)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
...
export default FinanceApprovedRequests;
