...
const DirectorOverview = () => {
  ...
  const fetchDashboardStats = async () => {
    setIsLoading(true);
    try {
      // Fetch pending reports (Institute Fund reports at School Chair Approved status)
      const { data: pendingData } = await API.get('/expense-reports?pending=true');
      
      // Fetch processed reports (Institute Fund reports that have been processed)
      const { data: processedData } = await API.get('/expense-reports?processed=true');
      
      // Calculate statistics
      const approved = processedData.filter(r => 
        r.directorApproval?.approved === true
      );
      
      const rejected = processedData.filter(r => 
        r.status === 'Rejected' && r.directorApproval?.approved === false
      );
      ...
      // Calculate total amount approved this month by this Director
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      const approvedThisMonth = approved.filter(r => {
        if (!r.directorApproval?.date) return false;
        const approvalDate = new Date(r.directorApproval.date);
        return approvalDate.getMonth() === currentMonth && 
               approvalDate.getFullYear() === currentYear &&
               r.directorApproval.approvedById === user._id;
      });
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div 
          ...
          onClick={() => navigate('/expense-reports/director/approvals')}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() => navigate('/expense-reports/director/approvals?tab=processed')}
              ...
              stats.recentReports.map((report) => (
                ...
                >
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(report.status)}`}>
                    {report.status === 'Dean SRIC Approved' ? 'Pending' : report.status}
                  </span>
                </div>
              ))
            )}
