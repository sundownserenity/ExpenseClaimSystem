...
const AuditOverview = () => {
  ...
  const fetchDashboardStats = async () => {
    setIsLoading(true);
    try {
      // Fetch pending reports (Director/Dean SRIC approved, waiting for Audit)
      const { data: pendingData } = await API.get('/expense-reports');
      
      // Fetch processed reports (Audit approved or rejected)
      const { data: processedData } = await API.get('/expense-reports?all=true');
      
      // Calculate statistics - only count reports this Audit user actually approved/rejected
      ...
      // Calculate total amount approved this month by this Audit user
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      const approvedThisMonth = approved.filter(r => {
        if (!r.auditApproval?.date) return false;
        const approvalDate = new Date(r.auditApproval.date);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div 
          ...
          onClick={() => navigate('/expense-reports/audit/approvals')}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() => navigate('/expense-reports/audit/approved')}
              ...
              stats.recentReports.map((report) => (
                ...
                >
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(report.status)}`}>
                    {(report.status === 'Director Approved' || report.status === 'Dean SRIC Approved') ? 'Pending' : report.status}
                  </span>
                </div>
              ))
            )}
      ...
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg shadow-sm border border-purple-100 p-6">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Financial Audit Department</p>
          </div>
        </div>
      </div>
    </div>
