...
const DeanSRICApprovedRequests = () => {
  ...
  const fetchApprovedReports = async () => {
    ...
    try {
      ...
      // Filter to show only reports approved by this Dean SRIC
      const myApprovedReports = data.filter(report => {
        const deanSRICApproval = report.deanSRICApproval;
        return deanSRICApproval && 
               deanSRICApproval.approved === true && 
               deanSRICApproval.approvedById === user._id;
      });
  ...
  const filterReports = () => {
    ...
    if (dateFilter !== 'all') {
      const now = new Date();
      filtered = filtered.filter(report => {
        const approvalDate = new Date(report.deanSRICApproval.date);
        switch (dateFilter) {
          case 'today':
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Approved Project Fund Requests</h1>
        <p className="text-gray-600 mt-2">
          Project Fund requests you have approved as Dean SRIC
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="Dean SRIC Approved">Pending Audit</option>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                    ...
                    <span className="flex items-center">
                      ...
                      Approved: {formatDate(report.deanSRICApproval.date)}
                    </span>
                  </div>
                  {report.deanSRICApproval.remarks && (
                    <div className="mt-2 text-sm">
                      <span className="font-medium text-gray-700">Your Remarks: </span>
                      <span className="text-gray-600">{report.deanSRICApproval.remarks}</span>
                    </div>
                  )}
                </div>
...
export default DeanSRICApprovedRequests;
